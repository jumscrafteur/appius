...
g = Game()
#BOUCLE INFINIE
#button = Button_text(screen_width/2, screen_height/2, lambda : print("ok"),"Je veux lancer le jeu")
running = True
button_start = Button_img(screen_width/2, screen_height/2, g.action, "assets/C3title_00001.png")
button_starte = Button_text(screen_width/2, 128 , 200, 100, lambda : print("ok"),"Start Game")
...
while running:
    ...
    if g.start_menu:
        ...
        screen.blit(pygame.transform.scale(fond, (screen_width, screen_height)), (0,0))
        #button.show(screen, True)
        button_load.show(screen, True)
        button_starte.show(screen, True)
        button_exit.show(screen, True)
        button_options.show(screen, True)
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        if event.type == pygame.MOUSEBUTTONDOWN:
            button_start.MouseonButton(event.pos)
            button_load.MouseonButton(event.pos)
